...
const enemiesList = [
    {name:"Mart", icon:"Mart.png"}, {name:"Baby", icon:"baby.png"}, {name:"NIL", icon:"nil.png"},
    {name:"Voidbreaker", icon:"voidbreaker.png"}, {name:"Springer", icon:"springer.png"},
    {name:"Bell", icon:"bell.png"}, {name:"Voidbound Guardian", icon:"vbg.png"},
    {name:"Skinwalker", icon:"skinwalker.png"}, {name:"Voidbound Baby", icon:"voidBaby.png"},
    {name:"???", icon:"random.png"}, {name:"Telefragger", icon:"tele.png"},
    {name:"Flesh", icon:"flesh.png"}, {name:"Guardian", icon:"guardian.png"},
    {name:"Cadence", icon:"Cadance.png"}, {name:"ICBM", icon:"icbm.png"}
];
...
function handlePlayerRules(){
 ...
 if(!isTrapMode){
   ...
   document.querySelectorAll(".upgrades-grid label").forEach(label=>{
     ...
     if(label.id === "opt-adrenaline"){ checkbox.disabled = p > 1; if(p > 1) checkbox.checked = false; }
     if(label.id === "opt-lrs"){ checkbox.disabled = p < 3; if(p < 3) checkbox.checked = false; }
   });
 }
 calculate();
}
...
function calculate(){
 ...
 if(isTrapMode){
   ...
   document.querySelectorAll(".option:checked").forEach(opt => {
     let price = (p === 1 && soloPrices[opt.parentElement.id]) ? soloPrices[opt.parentElement.id] : Math.ceil(Number(opt.dataset.price) * sqrtP);
